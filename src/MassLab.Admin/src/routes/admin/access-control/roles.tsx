...
function RolesPage() {
  ...
  const assigningRole = useMemo(
    () => roles.find((role) => role.id === assigningRoleId) ?? null,
    [assigningRoleId, roles],
  );
    ...
    <div className="space-y-6">
                      ...
                      onClick={async () => {
                        try {
                          await identityFetch<CommandResult>(
                            session,
                            `/api/admin/tenant/roles/${role.id}/delete`,
                            { method: "POST" },
                          );
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-lg bg-muted px-3 py-2">
                  ...
                  <div className="font-semibold">
                    {(assignedPermissionIds[role.id] ?? []).length}
                  </div>
                </div>
                <div className="rounded-lg bg-muted px-3 py-2">
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing?.id ? t("roles.edit") : t("roles.create")}
            </DialogTitle>
          </DialogHeader>
              ...
              onClick={async () => {
                ...
                try {
                  if (editing.id) {
                    await identityFetch<CommandResult>(
                      session,
                      `/api/admin/tenant/roles/${editing.id}/edit`,
                      {
                        method: "POST",
                        body: JSON.stringify({
                          name: editing.name,
      ...
      >
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-3 py-2">
            <div className="max-h-[60vh] overflow-y-auto">
              <PermissionTree
                ...
                rootLabel={assigningRole?.name ?? t("roles.permissions")}
                rootDescription={
                  assigningRole?.description || t("roles.editDesc")
                }
                searchPlaceholder={t("roles.searchPermissions")}
                selectedLabel={t("roles.selected")}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAssigningRoleId(null)}>
              {t("common.cancel")}
            </Button>
            <Button
              className="bg-gradient-brand text-primary-foreground"
              onClick={async () => {
                ...
                try {
                  await identityFetch<CommandResult>(
                    session,
                    `/api/admin/tenant/roles/${assigningRoleId}/permissions`,
                    {
                      method: "POST",
                      body: JSON.stringify({
...
async function loadRoles(
  ...
  const [rolesResult, usersResult] = await Promise.all([
    identityFetch<TenantRolesDto>(
      session,
      "/api/admin/tenant/roles?q=&sort=name&dir=asc",
    ),
    identityFetch<TenantUsersDto>(
      session,
  ...
  const counts = Object.fromEntries(
    rolesResult.roles.map((role) => [role.id, 0]),
  );
